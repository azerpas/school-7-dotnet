...
namespace Shard.Shared.Web.IntegrationTests
{
    public partial class BaseIntegrationTests<TEntryPoint, TWebApplicationFactory>
    {
        ...
        private static async Task AssertAttackedFirstAndNotSecond(HttpClient client, string path1, string path2)
        {
        ...
        private static async Task<(JObject, JObject)> GetDuelStatus(HttpClient client, (string, string) units)
        {
            return (await GetUnitOfPath(client, units.Item1), 
                await GetUnitOfPath(client, units.Item2));
        }

        private static async Task<JObject> GetUnitOfPath(HttpClient client, string path)
        {
        ...
        private static async Task AssertPathNotFound(HttpClient client, string path)
        {
            using var response = await client.GetAsync(path);
